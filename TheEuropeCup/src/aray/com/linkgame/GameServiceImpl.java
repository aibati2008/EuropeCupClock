...
//		int index = random.nextInt(4);
	...
	public Piece findPiece(float touchX, float touchY)
	{
		...
		// 第二个参数为每张图片的宽
		int indexX = getIndex(relativeX, IMAGE_WIDTH);
		// 获取relativeY座标在Piece[][]数组中的第二维的索引值
		// 第二个参数为每张图片的高
		int indexY = getIndex(relativeY, IMAGE_HEIGHT);
	...
	public LinkInfo link(Piece p1, Piece p2)
	{
		...
		{
			// 它们在同一行并可以相连
			if (!isXBlock(p1Point, p2Point, IMAGE_WIDTH))
			{
		...
		{
			if (!isYBlock(p1Point, p2Point, IMAGE_HEIGHT))
			{
				// 它们之间没有真接障碍, 没有转折点
				return new LinkInfo(p1Point, p2Point);
			}
